...
# initiate log file
logfile = open("/Users/tobias/GitHub/bayesian_modeling_course/data/py_mcmc_samples_gamma_factor_update_1-5.txt","w")
...
current_prior_shape = pdf_gamma(shape_init,shape_prior_mu,shape_prior_sd)
current_prior_rate = pdf_gamma(rate_init,rate_prior_mu,rate_prior_sd)
...
for i in range(0,n_iterations):
    ...
    new_prior_shape = pdf_gamma(proposed_shape,shape_prior_mu,shape_prior_sd)
    new_prior_rate = pdf_gamma(proposed_rate,rate_prior_mu,rate_prior_sd)
...
import pandas as pd
mcmc_log = pd.read_csv("/Users/tobias/GitHub/bayesian_modeling_course/data/py_mcmc_samples_gamma.txt", sep = '\t')
...
data = mcmc_log['shape'][1:]
sns.set_style('whitegrid')
sns.kdeplot(np.array(data), bw=4,color="green", shade=True)
plt.show()

data = mcmc_log.rate[1:]
sns.set_style('whitegrid')
sns.kdeplot(np.array(data), bw=0.05,color="blue", shade=True)
plt.show()
